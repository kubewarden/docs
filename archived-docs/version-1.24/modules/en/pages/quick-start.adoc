...
* {cluster-admission-policy} resources: this is how policy definition happens
  for Kubernetes clusters.
* {policy-server} resources: representing a deployment of a {project-name}
  `PolicyServer`. The Kubewarden `PolicyServer` loads and evaluates your
  administrator's policies.
* {admission-policy} resources: policies for a defined namespace.
* A deployment of a `kubewarden-controller`: this controller monitors the
  {cluster-admission-policy} resources and interacts with the {project-name}
  {policy-server} components.

[TIP]
====

{project-name} describes its Kubernetes Custom Resource Definitions (CRDs)
xref:reference/CRDs.adoc[here].

{project-name} CRDs mentioned in this tutorial and in the rest of
...
You can retrieve {project-name} policies from the GitHub container registry at
https://ghcr.io. You need authentication to use the repository with the
{project-name} CLI, a {gh-pat-url}[GitHub personal access token] (PAT). Their
...
Deploy the {project-name} stack using `helm` charts as follows:
...
* `kubewarden-controller`, which installs the {project-name} controller and the
  audit scanner
+
[NOTE]
====
...
a `PolicyServer` resource named `default` isn't created when using the
...
you try to install the `kubewarden-defaults` with certain conflicting
information, for example, the same policy server name. To be able to use future
upgrades in the `kubewarden-defaults` Helm chart remove the existing
...
upgrades without resource conflicts. When you remove the `PolicyServer`, you
remove all the policies bound to it too.
...
{project-name} has three main components which you interact with:
...
The `kubewarden-controller` manages a {project-name} `PolicyServer`. You can
deploy multiple {policy-server}s in the same Kubernetes cluster.

A `PolicyServer` validates incoming requests by executing {project-name}
...
| The name of the `ServiceAccount` to use for the `PolicyServer` deployment. If
  no value is provided, the default `ServiceAccount` from the namespace, where
  the `kubewarden-controller` is installed, will be used.
...
The {cluster-admission-policy} resource is the core of the {project-name}
...
| N
| `policy-server`
| Identifies an existing `PolicyServer` object. The policy will be served only
  by this `PolicyServer` instance. A {cluster-admission-policy} that doesn't
  have an explicit `PolicyServer`, will be served by the one named `default`.

| Y
| `module`
| The location of the {project-name} policy. The following schemes are allowed:

| N
|
| - `registry`: The policy is downloaded from an
...
| N
|
| - `file`: The policy is loaded from a file in the computer file system.
...
| Y
| `mutating`
| A boolean value that must be set to `true` for policies that can mutate
  incoming requests
...
| N
|
| - `Ignore`: an error calling the webhook is ignored and the API request
...
The {cluster-admission-policy} resources are registered with a `*` webhook
`scope`, which means that registered webhooks forward all requests matching the
...
When defining a {cluster-admission-policy}, the status becomes `pending`, and it
forces a rollout of the targeted `PolicyServer`. In our example, it's the
`PolicyServer` named `default`. You can monitor the rollout by running the
following command:
...
The {cluster-admission-policy} status becomes `active` once the Deployment is
done for every `PolicyServer` instance. Show
...
{project-name} contains a helm pre-delete hook that removes all
...
Now, you are ready to deploy {project-name}! Have a look at the policies on
