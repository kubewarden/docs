...
* {policy-server} resources: representing a deployment of a {product-name}
...
{product-name} describes its Kubernetes Custom Resource Definitions (CRDs)
xref:reference/CRDs.adoc[here].

{product-name} CRDs mentioned in this tutorial and in the rest of
...
You can retrieve {product-name} policies from the GitHub container registry at
https://ghcr.io. You need authentication to use the repository with the
{product-name} CLI, a {gh-pat-url}[GitHub personal access token] (PAT). Their
...
{product-name} has three main components which you interact with:
...
The `kubewarden-controller` manages a {product-name} `PolicyServer`. You can
deploy multiple {policy-server}s in the same Kubernetes cluster.

A `PolicyServer` validates incoming requests by executing {product-name}
...
The {cluster-admission-policy} resource is the core of the {product-name}
...
{product-name} contains a helm pre-delete hook that removes all
