...
Kubewarden implements support for the https://www.sigstore.dev/[Sigstore]
...
check the documentation on xref:/howtos/secure-supply-chain.adoc[how
Kubewarden implement a Secure Supply Chain for the policies].
...
Obtaining and operating with those OCI layers needs to happen outside the
...
Both functions verify that the image signature satisfies the constraints.

[NOTE]
====

On success, the functions return the digest of the verified image. It is now
the policy responsibility to check that containers instantiation is from that
...
The Kubewarden team
https://github.com/kubewarden/verify-image-signatures[provides a verifier
policy] that enforces Sigstore signatures for all containers. It's built using
Rust and the Rust SDK. The policy checks container signatures, and optionally,
mutates the requests. The mutation appends the verified checksum to the tag of
the image. Check the documentation for specifics.
