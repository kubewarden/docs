...
{product-name} uses the terms
'PolicyServer' when discussing the {product-name} policy server
and
`policy-server` when discussing Pod or Deployment of a {product-name} PolicyServer.
====
...
{product-name} operates seamlessly within the Kubernetes ecosystem.
At its core, the {product-name} controller is a Kubernetes controller,
monitoring {product-name} Custom Resource Definitions (CRDs)
and configuring Kubernetes resources to execute them.
This integration ensures that {product-name} uses the built-in Kubernetes mechanisms,
...
When setup by the {product-name} controller,
...
This cross-platform compatibility ensures that {product-name} policies are versatile,
and can be distributed and executed in diverse Kubernetes clusters

=== OCI based policy artifacts

Policies in {product-name} are OCI (Open Container Initiative) artifacts.
...
{product-name} associates policies with their own 'validation' or 'mutating' webhook,
...
In particular, {product-name} operates as a Kubernetes Admission Webhook.
The `policy-server` is the Webhook endpoint called by the Kubernetes API server to validate requests.

* The https://github.com/kubewarden/kubewarden-controller[{product-name} controller]
is a Kubernetes controller that reconciles {product-name}'s Custom Resources.
This controller creates parts of the {product-name} stack.
...
* xref:../tutorials/writing-policies/index.adoc[{product-name} policies]
...
{product-name} works as a Kubernetes Admission Webhook.
...
and downloads all the {product-name} policies specified.
You can download {product-name} policies from remote HTTP servers and container registries.
...
When {product-name} has configured all policies,
the `policy-server`
spawns a pool of worker threads to evaluate incoming requests
using the {product-name} policies specified by the user.

Finally, the `policy-server` starts a HTTPS server,
listening to incoming validation requests.
{product-name} uses the TLS key and certificate
created by the {product-name} controller
...
Once {product-name} marks the `policy-server` deployment as 'uniquely reachable' or `Ready`,
...
{product-name} evaluates each policy inside its own dedicated WebAssembly sand-box.
