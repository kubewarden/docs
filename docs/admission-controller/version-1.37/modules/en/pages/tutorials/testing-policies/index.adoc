include::partial$variables.adoc[]
= Policy testing
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: Learn how to create and test {admc-name} Policies with {admc-name} for secure cluster management.
:doc-persona: ["kubewarden-operator", "kubewarden-policy-developer"]
:doc-topic: ["testing-policies", "introduction"]
:doc-type: ["tutorial"]
:keywords: ["kubewarden", "persons", "policy author", "cluster operator"]
:sidebar_label: Policy testing
:current-version: {page-origin-branch}

== Policy testing personas

This section covers the topic of testing {admc-name} Policies.
There are two personas interested in testing policies:

* *Policy developer*: you're writing a {admc-short-name} Policy and you want to make sure your code behaves the way you expect.
* *Cluster operator*: you found a {admc-short-name} Policy and you want to test and tune the policy settings before deploying it.
You want to keep testing these settings in your CI/CD pipelines, and other administrative tasks.

The next sections of the documentation shows how these personas can test {admc-short-name} policies.
