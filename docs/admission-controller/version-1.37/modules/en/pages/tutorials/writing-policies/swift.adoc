...
In the meantime, you can use the tool chain provided by the SwiftWasm project to build {admc-name} policies.
...
* https://github.com/kubewarden/policy-sdk-swift[{admc-short-name} Swift SDK]: this provides a set of ``struct``s and functions that simplify the process of writing policies.
* https://github.com/kubewarden/swift-policy-template[{admc-short-name} Swift template project]: use this template to scaffold a Swift-based policy.
...
has a {admc-short-name} Policy written in Swift.
