...
There's another Go compiler that can build WebAssembly binaries usable by {admc-name}.
...
The {admc-short-name} project currently suggests using TinyGo for two reasons:
...
{admc-short-name} policies need to process JSON data such as policy settings and the request received by Kubernetes.

Despite TinyGo's current limitations, it's still easy to write {admc-short-name} validation policies with it.

== Tooling

Writing {admc-short-name} policies requires a version of TinyGo greater than `v0.28.1`.
...
These Go libraries are useful when writing a {admc-short-name} policy:
...
Set `union`, `intersection`, `difference` are common operation in {admc-short-name} policies.

Lastly, the {admc-short-name} project provides a
https://github.com/kubewarden/go-policy-template[template Go policy project]
you can use to create {admc-short-name} Go policies.
...
CLI tool provided by {admc-short-name} to run its policies outside of Kubernetes, among other actions.
