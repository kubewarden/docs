...
All existing Gatekeeper policies should be compatible with {admc-short-name} as explained in this chapter.

[IMPORTANT]
====
If you find this not to be true, for your Gatekeeper policies,
report it,
and we'll work to ensure your Gatekeeper policy runs with {admc-short-name}.
====
...
xref:tutorials/writing-policies/rego/02-builtin-support.adoc[built-in support] in {admc-short-name}.
