...
Both OPA Gatekeeper and Kubewarden, from which {admc-name} is derived, are open source projects,
and part of CNCF.

This table provides a comparison between OPA Gatekeeper and {admc-short-name}. Topics
requiring more information have links to further explanation.

|===
| Feature | OPA Gatekeeper | {admc-short-name}
...
You can write {admc-short-name} policies using different paradigms. Policy authors can
...
Languages] like Rego and CEL. In {admc-short-name}'s you write validating and mutating
...
{admc-short-name} can use Rego policies written for both Open Policy
...
Both OPA Gatekeeper and {admc-short-name} support these types of policies.
...
{admc-short-name} has two different types of Custom Resources used to declare
...
OPA Gatekeeper and {admc-short-name} policies have policy source code (Rego for OPA
Gatekeeper, CEL for {admc-short-name}) in the Custom Resource defining a policy in
Kubernetes.

Also, {admc-short-name} policies can have the source code of the policy managed like
...
Both OPA Gatekeeper and {admc-short-name} are managed using GitOps methodologies.
...
Both OPA Gatekeeper and {admc-short-name} can deploy policies using two different
operation modes:
...
{admc-short-name} allows definition of multiple evaluation servers. You define these
...
Both OPA Gatekeeper and {admc-short-name} have a scanner that operates in the
background. This scanner evaluates resources already defined in the cluster and
flags non-compliant ones.

The only difference between OPA Gatekeeper and {admc-short-name} is how the scanner
results get saved.
...
{admc-short-name} instead stores the results inside of a set of the Reports
