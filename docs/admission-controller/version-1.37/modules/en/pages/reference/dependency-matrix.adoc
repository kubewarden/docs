...
This page lists the dependencies of {admc-name}, with their relevant
...
backwards-incompatible changes still happen. The {admc-short-name} developers endeavor
...
| `≥ 2 < 4`
| In `admission-controller` chart as subchart
| Policy Reports UI
|===
...
| `v1beta1`
| In `admission-controller` chart or manually installed
| Audit Scanner
...
{admc-short-name} tests against the monitoring and tracing Helm charts provided in the
