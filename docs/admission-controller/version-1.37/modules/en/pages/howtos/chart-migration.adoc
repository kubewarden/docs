...
. *Preflight*: checks that the required tools are available, connects to the
  cluster, finds the three legacy releases, and warns if they are not at the
...
. *Unified chart install*: runs `helm install --take-ownership` as a fresh
  release named `admission-controller`, with no name/fullname overrides. Helm
  4's Server-Side Apply adopts the kept data/identity resources, updates their
  ownership metadata, and removes the legacy keep annotations; the controller
  plumbing is created fresh under the chart's natural names.
...
| `--interactive`
| Pause for confirmation before destructive steps
