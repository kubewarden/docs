...
You can now install {admc-short-name}, in the recommended way with Helm
charts.
...
First, you should add the Helm repository that contains {admc-short-name}:
...
Now you can deploy the rest of the {admc-short-name} stack. The official
Helm chart creates a PolicyServer named `default`.

You should configure the Helm chart so that you have metrics enabled
in {admc-short-name}. The `kubewarden-values.yaml` file should have the following contents:
...
=== Using the {admc-short-name} Grafana dashboard

{admc-short-name} has a Grafana dashboard with basic metrics giving an
overview about how {admc-short-name} behaves in the cluster. This dashboard
is available in the GitHub releases of the {admc-short-name} policy-server
repository as a
...
{admc-short-name}. It also has policy-specific panels.
