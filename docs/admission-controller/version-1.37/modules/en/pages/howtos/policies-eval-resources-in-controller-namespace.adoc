...
could break the {admc-short-name} installation.
...
To do this, upgrade your {admc-short-name} installation and set the
...
helm upgrade -n kubewarden
  ...
  kubewarden-controller kubewarden/admission-controller
----
...
from the {admc-short-name} namespace.
...
skips the {admc-short-name} namespace, the policy will still ignore that namespace as
expected.
====
