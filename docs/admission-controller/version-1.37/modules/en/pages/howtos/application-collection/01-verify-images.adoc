...
https://apps.rancher.io/[Rancher Application Collection] with {admc-name}.
...
For PolicyServer `default` from the `admission-controller` Helm chart, it would
be:
...
If using the PolicyServer `default` from the `admission-controller` Helm chart,
...
$ helm upgrade -i --wait --namespace kubewarden \
  --create-namespace admission-controller kubewarden/admission-controller \
  --reuse-values \
