...
{admc-name} stack webhooks when using https://k3s.io/[K3s] as your
...
{admc-short-name} webhooks and the Kubernetes API server.
...
needs to be available to the {admc-short-name} stack.
...
Install the {admc-short-name} stack as described in the
...
{admc-short-name} stack and its webhooks.
