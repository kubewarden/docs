...
:description: Use Hauler to install {admc-name} in air-gapped environments.
...
This guide shows you how to install {admc-name} in air-gapped environments using
...
{admc-short-name} provides a manifest file with all the
required resources to run {admc-short-name} in a private environment. This
...
. Get it from a {admc-short-name} release page
. Load all the resources required to run {admc-short-name} into a local store
...
Download `hauler_manifest.yml` from the {admc-short-name}
...
== Generate the tarball file with all {admc-short-name} resources
...
{admc-short-name} container images support x86_64 and Arm architectures. Therefore,
when you save the resource into the file, you may see warning messages like
these:

[source,shell]
----
2025-08-12 18:55:54 WRN specify an export platform to prevent potential platform mismatch on import of index [ghcr.io/kyverno/policy-reporter:3.8.1]
2025-08-12 18:55:54 WRN specify an export platform to prevent potential platform mismatch on import of index [ghcr.io/kyverno/policy-reporter-ui:2.6.1]
2025-08-12 18:55:54 WRN specify an export platform to prevent potential platform mismatch on import of index [ghcr.io/kubewarden/adm-controller/policy-server:v1.37.0]
2025-08-12 18:55:54 WRN specify an export platform to prevent potential platform mismatch on import of index [ghcr.io/kubewarden/adm-controller/audit-scanner:v1.37.0]
2025-08-12 18:55:54 WRN specify an export platform to prevent potential platform mismatch on import of index [ghcr.io/rancher/kuberlr-kubectl:v8.1.1]
2025-08-12 18:55:54 WRN specify an export platform to prevent potential platform mismatch on import of index [ghcr.io/kubewarden/adm-controller/controller:v1.37.0]
----
...
Now that you have all the {admc-short-name} resources in
...
Now all the resources required to install {admc-short-name} are in the Hauler store in
...
# helm install .. admission-controller .. --set policyServer.insecureSources[0]=<HOST_IP:5000>
...
modules and Helm charts used by {admc-short-name} into your private registry.

== Install {admc-short-name}

Now that your private registry has everything required, you can install
{admc-short-name}. The difference from a standard {admc-short-name}
...
[source,shell]
----
helm install --wait -n kubewarden admission-controller --create-namespace \
    --set "global.cattle.systemDefaultRegistry=<REGISTRY.YOURDOMAIN.COM:PORT>" \
    oci://<REGISTRY.YOURDOMAIN.COM:PORT>/hauler/admission-controller
----

[CAUTION]
====

To use the PolicyReporter sub-chart available in the `admission-controller`
chart you need to define other values specific for the sub-chart in an
air-gapped environment. See an example below:

[source,shell]
----
helm install --wait -n kubewarden admission-controller oci://<REGISTRY.YOURDOMAIN.COM:PORT>/hauler/admission-controller \
    --set global.cattle.systemDefaultRegistry=<REGISTRY.YOURDOMAIN.COM:PORT> \
...
Now create {admc-short-name} policies in your cluster. Policies must be
...
spec:
  module: registry://<REGISTRY.YOURDOMAIN.COM:PORT>/kubewarden/policies/pod-privileged:v1.0.11
  rules:
  - apiGroups: [""]
    apiVersions: ["v1"]
    resources: ["pods"]
...
spec:
  image: <REGISTRY.YOURDOMAIN.COM:PORT>/kubewarden/adm-controller/policy-server:v1.37.0
  replicas: 2
