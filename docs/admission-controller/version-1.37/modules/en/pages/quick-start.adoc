...
* `kubewarden-controller`, which installs the {admc-short-name} and the
  audit scanner
+
[NOTE]
====
...
{admc-short-name} has three main components which you interact with:
...
deploy the {admc-short-name} stack:
...
{admc-short-name} contains a helm pre-delete hook that removes all
...
{admc-short-name} deletes {validating-webhook-configuration}s and
...
Now, you are ready to deploy {admc-short-name}! Have a look at the policies on
