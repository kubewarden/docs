...
Let's see a concrete example. These and others can be reused with {admc-name}'s
...
Here, we've an equivalent {admc-short-name} policy:

### {admc-short-name}'s `cel-policy`
...
| 1   | `failurePolicy`                   | `settings.failurePolicy`                 | CEL behavior, for when CEL expression evaluates to false, there's CEL runtime errors, or there's invalid or misconfigured CEL. For example, a CEL expression returning false, missing parameters, or missing variables. Not to confuse with (6).
...
| 3   | `variables`                       | `settings.variables`                     | In {admc-short-name}'s `cel-policy`, expressions that define variables are in `settings.variables`. Apart from that, they are equivalent.
| 4   | `paramKind`,`paramRef`            | `settings.paramKind`,`settings.paramRef` | In {admc-short-name}'s `cel-policy`, parameter definitions are in `settings.paramKind`, `settings.paramRef`. Apart from that, they are equivalent.
| 5   | `validations`                     | `settings.validations`                   | In {admc-short-name}'s `cel-policy`, expressions that define validations are in `settings.validations`. Apart from that, they are equivalent.
...
| 7   | `validationActions`               | `mode`                                   | `mode` has as options `protect` and `monitor`. Auditing is more full featured in {admc-short-name}, see (9).
| 8   | `matchResources`                  | `namespaceSelector`, `objectSelector`    | Define ways to constraint using Selectors. {admc-short-name}'s policies have them as `namespaceSelector` and `objectSelector`.
| 9   | `auditAnnotations` (not pictured) | `backgroundAudit`, annotations           | Use {admc-short-name} fields instead to set the policy usage in xref:explanations/audit-scanner/audit-scanner.adoc[Audit Scanner], and its category and severity for OpenReports.
|10|`---`|`contextAwareResources` permissions|{admc-short-name}}'s policies have fine-grained permissions for reading cluster Resources. Here it is used for reading the params.
|     | `matchConditions`                 | `matchConditions`                        | {admc-short-name}'s policies have `matchConditions` (not pictured in this example).
|     | `---`                             | {admc-short-name}-only features                 | For other features, see the rest of tutorial CEL examples.
|===

[TIP]
====

You can use the `kwctl` tool to migrate a VAP policy to {admc-short-name}.
...
This is covered by {admc-short-name}'s xref:explanations/audit-scanner/audit-scanner.adoc[Audit Scanner] and PolicyReports, which allows
...
limit].
This is covered by {admc-short-name}'s general xref:reference/policy-evaluation-timeout.adoc[policy timeout
protection].
