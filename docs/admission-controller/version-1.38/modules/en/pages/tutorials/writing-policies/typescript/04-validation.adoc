...
:doc-type: tutorial
:doc-topic: {admc-name}, kubewarden, writing-policies, typescript, validation-logic
...
Writing to STDOUT breaks policies. Instead, use STDERR for logging or the logging facility provided by the {admc-name} SDK. The policy's output to STDOUT must only contain the validation response.
====
...
You can access this JSON object data through the {admc-short-name} SDK's helper functions.
...
The {admc-short-name} Javy plug-in provides the `policyAction()` function indicating whether the policy should validate a resource or validate its settings.