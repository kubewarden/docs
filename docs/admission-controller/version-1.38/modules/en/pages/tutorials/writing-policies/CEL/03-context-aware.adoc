...
{admc-name}'s `cel-policy` supports the xref:reference/spec/05-context-aware-policies.adoc[context
...
To achieve this, we can use the https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name}'s CEL extension
...
the cluster. For that, we use the
https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{admc-short-name}
CEL extension library]. Particularly, the `kw.k8s` host capabilities, which
allows us to query the cluster for GroupVersionKinds. You can see the
available docs for the CEL functions
