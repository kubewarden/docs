...
The {admc-name} stack uses webhooks to enforce policies in a Kubernetes cluster.
...
This network policy uses label selectors introduced in {admc-short-name} 1.23.0. If
...
The webhooks exposed by the {admc-short-name} stack should only accept requests from
...
. Create the `kubeconfig` file the admission plug-ins refer to. {admc-short-name} only
...
certificate available to the {admc-short-name} stack.
+
...
install the `admission-controller` is:
+
[source,console]
----
...
----
+
The {admc-short-name} controller creates a client certificate for use by the audit
