...
The `default` Policy Server deployed by {admc-name} Helm charts has access to
...
| Rego
| ✅
| Since {admc-short-name} 1.9 release

| WASI
| ✅
| Since {admc-short-name} 1.10.0 release, only for Go SDK
|===

== Constraints

{admc-short-name}'s priority is to reduce the number of queries against the Kubernetes
API server. {admc-short-name} considers two restraints:
...
{admc-short-name} policies can fetch multiple resources at the same time. For example,
...
query. This limitation is to be removed in future releases of {admc-short-name}.

=== Fetching a specific resource

{admc-short-name} policies can get a specific resource defined inside of the cluster.
...
policy author using the {admc-short-name} SDKs.

== ClusterAdmissionPolicies

ClusterAdmissionPolicies have the field
xref:/reference/CRDs.adoc#contextawareresource[spec.contextAwareResources].
