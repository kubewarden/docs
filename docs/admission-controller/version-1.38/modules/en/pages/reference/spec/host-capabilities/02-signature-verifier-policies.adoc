...
{admc-name} implements support for the
...
{admc-short-name}'s `policy-server` or `kwctl`.

The different language SDKs for {admc-short-name} policies take care of this.
They provide functions for verification of container image,
{admc-short-name} policies, Helm charts and other kinds of OCI artifact.
