...
During evaluation, {admc-name} policies can access extra capabilities offered by the host environment.
This mechanism uses an approach similar to traditional
https://en.wikipedia.org/wiki/Remote_procedure_call[RPC].

This is what happens when a request is issued by a {admc-short-name} policy:

* {admc-short-name} policy:
