...
Model for Kubernetes. The {admc-name} team continuously evaluates {admc-short-name}
against this threat model, and works to provide secure defaults. It's
recommended that {admc-short-name} administrators read and understand the threat
...
This is {admc-short-name}'s default behavior.

Failing closed means that if, for any reason,
{admc-short-name} stops responding or crashes,
the API server rejects the request by default.
This is even if the request is normally accepted by {admc-short-name}.
...
following is coming in due course, to help {admc-short-name} users:
...
Since the webhook uses TLS encryption for all traffic, {admc-short-name} is safe.
...
mTLS feature in the {admc-short-name} stack. Alternatively, setup mTLS using a CNI
...
Enable the recommended policies, which includes the `hostpaths-psp` policy.
This policy is configured to reduce the shared hostPath volumes.
...
* Configure the cluster with mTLS authentication for the Webhooks and enable
  the mTLS feature in the {admc-short-name} stack. Alternatively, setup mTLS using a
  CNI that supports Network Policies.

* By default, {admc-short-name} policies don't have network access and run in a
  restrictive environment, strictly controlling external access on Webhooks.

== {admc-short-name} threats

=== {admc-short-name} threat 1 - Bootstrapping of trust for admission controller

==== Scenario

Assuming a trusted but new Kubernetes cluster, an attacker is able to
compromise the {admc-short-name} stack before deployment and enforcement any of the
policies securing.
 ...
 ** policy-server
 ** any of the {admc-short-name} dependencies
 ** any optional dependencies (Grafana, Prometheus, and others)
* by compromising the Helm charts payload

==== Mitigation

. {admc-short-name} provides a Software Bill Of Materials, which lists all images
  needed. This aids with Zero-Trust. The Kubernetes Administrator must verify
  the {admc-short-name} images, its dependencies' images, and charts out of the
...
. Use signed Helm charts, and verified digests instead of tags for {admc-short-name}
  images in those Helm charts. This doesn't secure dependencies though.

=== {admc-short-name} threat 2 - Attacker uses namespaced policy to extract information
...
* Restrict the resources that namespaced policies can target. Use the
  `namespacedPoliciesAllowedResources` Helm value. See
...
See xref:/howtos/policy-servers/06-namespaced-policies-capabilities.adoc[Controlling host capabilities for namespaced policies]
for full configuration details.

=== {admc-short-name} threat 3 - PolicyServer compromise exposes all its mapped namespaces
...
* Apply the principle of least privilege to
  `namespacedPoliciesAllowedResources`. Grant only the resources that
  the namespaced policies on that instance need to target.
...
=== {admc-short-name} threat 4 - Stale or outdated Namespace-to-PolicyServer mapping
