...
{admc-name} policies are WebAssembly (Wasm) binaries evaluated by the {admc-short-name} Policy Server.

The {admc-short-name} policy server loads policies from these sources:
...
{admc-short-name} recommends distributing {admc-short-name} policies via a regular
...
a {admc-short-name} Policy to it.

You can use the xref:/howtos/install-kwctl.adoc[`kwctl`] CLI to push a
{admc-short-name} Policy to an OCI-compliant registry.

== Annotating the policy

You also annotate a policy with `kwctl`. Annotating a {admc-short-name}
...
* the {admc-short-name} policy to annotate, a local file in the filesystem.
...
You can reference a policy from the {admc-short-name} Policy Server or `kwctl` as
