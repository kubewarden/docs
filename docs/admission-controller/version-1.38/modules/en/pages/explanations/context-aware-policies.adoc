...
:keywords: ["{admc-name}", "kubewarden", "context-aware policy", "clusteradmissionpolicy", "admissionpolicy", "admissionrequest", "cluster administrator"]
...
[NOTE]
====
Context aware policies are only available in {admc-name} versions ≥ v1.6.0.
====
...
By default, {admc-short-name} blocks all cluster resources. A {admc-name} administrator
...
policy. Policy authors do this by annotating the policy. {admc-name}
...
To prevent system abuse, {admc-name} administrators must review the resources
the policy accesses.
