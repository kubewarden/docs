...
created by the {admc-short-name} controller to let Kubernetes know where to send an `AdmissionReview`.
In other words,
this is how a {admc-short-name} controller informs Kubernetes where to find a resource mutating policy.
...
A PolicyServer validates incoming requests by executing {admc-short-name} policies against requests.
...
A
https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/#what-are-admission-webhooks[Kubernetes resource]
created by the {admc-short-name} controller to let Kubernetes know where to send a `AdmissionReview`.
In other words, this is how {admc-short-name} informs Kubernetes where to find a resource validating policy.
