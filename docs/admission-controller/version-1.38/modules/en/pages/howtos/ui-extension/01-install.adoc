...
This page is under construction. The install process may vary.

====


This section describes installing the {admc-name} UI as an extension
of https://github.com/rancher/rancher[Rancher Manager].

== Install {admc-short-name} UI Extension

You install the {admc-short-name} UI as a global extension, however, you
install the {admc-short-name} controller through the Rancher UI as a cluster
scoped resource.

[NOTE]
====

For air-gapped installations, follow xref:howtos/airgap/01-requirements.adoc[these
steps].
...
add the Rancher Extensions Repository. When enabled, the "{admc-short-name}"
extension item appears automatically. Select this item to install the
extension. Once installed, you can install {admc-short-name} into the
required cluster.

=== Install {admc-short-name}

Following the previous steps, within your cluster a new item appears in the
side-menu for {admc-short-name}. This dashboard page guides you through the
...
{admc-short-name}" button may remain grayed out. If this happens, refresh
...
access {admc-short-name}'s official policies.
...
As {admc-short-name} is a Rancher Official Extension, the Rancher team
...
Helm repository with the {admc-short-name} UI chart for installation.
...
. Select the *Available* tab, and click the *Reload* button to make sure that
  ...
  {admc-short-name}]
. Find the {admc-short-name} extension you just added, and select the
  *Install* button.
