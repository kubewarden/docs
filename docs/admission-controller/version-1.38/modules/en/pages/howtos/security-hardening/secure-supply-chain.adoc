...
:doc-type: ["howto"]
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "kubernetes", "secure supply chain", "infrastructure"]
...
{admc-short-name} uses https://github.com/sigstore/cosign[`cosign`] together
...
Cluster operators can configure {admc-name} to only run policies
...
xref:/reference/kwctl-cli.adoc[`kwctl`] utilities for signing and
inspecting policies.
...
{admc-short-name} recommends using Sigstore's
...
policy is ready to use in a {admc-short-name} installation using signature
verification.
...
is an example of how the {admc-short-name} project signs its policies:

.YAML describing {admc-short-name} policy signing
[%collapsible]
======
...
Policy developers can use the {admc-short-name} policy templates. They have GitHub
...
You can configure {admc-short-name} with a `ConfigMap` to only run trusted
...
For example, you want to run policies signed by the {admc-short-name} GitHub
...
If you deploy the default Policy Server using the `admission-controller` Helm
...
With {admc-short-name}, you can check the policy signature is by trusted
...
This last example tests if a given policy came from the {admc-short-name} organization:
