...
:doc-type: ["howto"]
:keywords: ["{admc-name}", "kubewarden", "pod security admission", "pod security policy", "kubernetes"]
...
{admc-short-name} can be used to *integrate* a PSA profile to avoid these limitations.

[NOTE]
====

You could use {admc-short-name} to replace the old PSP configuration as shown in xref:howtos/psp-migration.adoc[PSP migration].
However, the goal of this article is to show how {admc-short-name} can complement the new PSA.
====
...
This is where {admc-short-name} can help.
...
You need the {admc-short-name} stack in the Kubernetes cluster for this example.
...
Now the deployment is mutated by {admc-short-name}'s policy which allows the Pod to be started.
...
The {admc-short-name} integration can do more in this scenario.
It can check the value of the `runAsUser` provided.

This resource is rejected by the {admc-short-name} policy from earlier:
...
Using {admc-short-name} together with PSA helps fill this gap.
