...
{admc-name} provides features for reliability and correct scheduling of its
components in a Kubernetes cluster. Some of the hints on this page come from
{admc-short-name} community members using {admc-short-name} at scale.

[IMPORTANT]
====
If you want to see a real example of running {admc-short-name} at scale
check out the xref:/howtos/deploy-at-scale.adoc[{admc-short-name} in a
...
fine-tune the scheduling and reliability of the {admc-short-name}
...
Starting from version 1.15 of the {admc-short-name} stack, the
{admc-short-name} Helm chart ships with two new values:

* `.global.tolerations`
* `.global.affinity`

These Helm chart values allow users to define Kubernetes tolerations
and affinity/anti-affinity settings for the {admc-short-name} stack,
...
tolerations for the {admc-short-name} components. Tolerations allow
...
anti-affinity rules for the {admc-short-name} components. Affinity rules
...
for the workload pods of the {admc-short-name} stack. This ensures the
{admc-short-name} workload is available over other workloads, preventing
...
This is available via the `.global.priorityClassName` value in the Helm chart.
...
{admc-short-name} provides several ways to increase the reliability of
...
The {admc-short-name} controller can create a
...
The {admc-short-name} controller can set the affinity of
...
The {admc-short-name} controller can set the resource limits and
...
[NOTE]
====
The `memory` limit in this example applies to the full `PolicyServer`
Pod. This limit is different from the policy memory limit, which has a
default value of 256 MiB. See
xref:/reference/policy-evaluation-timeout.adoc#_policy_memory_limit[Policy
memory limit].

The Pod `memory` limit must be sufficient for all the policies that run
on the `PolicyServer`, at the expected number of concurrent evaluations.
====

=== PriorityClasses

The {admc-short-name} controller can set the PriorityClass used for the
