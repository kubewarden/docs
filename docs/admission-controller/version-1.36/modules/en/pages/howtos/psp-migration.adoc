...
:description: Configure {admc-name} policies in your Kubernetes clusters to achieve granular control over policy configuration, replacing PodSecurityPolicy (PSP).
...
(PSP) is removed. Now you can use {admc-name} for admission control on your
Kubernetes clusters.

{admc-short-name} has separate policies to achieve the same goal as a monolithic PSP
configuration. Each {admc-short-name} policy definition functions as a different
configuration section in the specification of a PSP. The mapping of PSP
configuration fields to their respective {admc-short-name} policies is in the
<<_mapping_policies_to_psp_fields,mapping table>> below.

With {admc-short-name}, operators have granular control of policy configuration in
their clusters.

With a {admc-short-name} instance, you can deploy policies to replace the
...
== {admc-short-name} replacements for PSP

Now we will create {admc-short-name} policies to achieve the same goal.
You enforce each rule with a separate {admc-short-name} policy.
...
== Mapping {admc-short-name} policies to PSP fields

This table maps PSP configuration fields to corresponding {admc-short-name} policies.

|===
| PSP field | {admc-short-name} equivalent policy
...
The {admc-short-name} team has developed a script for PSP migration. It uses the
...
policies. It does this for {admc-short-name} and other policy engines.

[CAUTION]
====

The AppVia migration tool is out of control of the Kuberwarden maintainers.
This means that it's possible it generates out-of-date {admc-short-name} policies. Use
with caution. We need a pull request for AppVia for which work is ongoing.
Contact us for more information if you need to.
====


The script is available in the {admc-short-name}
...
{admc-short-name} policies can be applied directly to a cluster using the following command:
