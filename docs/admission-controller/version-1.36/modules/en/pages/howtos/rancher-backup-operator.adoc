...
Since version `v9.0.0`, `rancher-backup` has support for {admc-name}. This
includes:

- The default Rancher Namespace `cattle-kubewarden-system` (or
  `cattle-kubewarden-*`), and the default {admc-short-name} Namespace
  `kubewarden`.
- {admc-short-name} needed resources are installed via the Helm charts.
- {admc-short-name} CRDs, which get reconciled after restore by the
...
One needs to instruct the Backup Operator to also backup {admc-short-name}
...
Use the `rancher-resource-set-full` to backup the {admc-short-name} Secrets.
