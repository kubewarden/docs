...
This page is for {admc-name} maintainers or low level policy authors who want to experiment with bleeding edge WASM platforms.
====
...
Many of the compilers used to compile {admc-short-name} policies produce WebAssembly modules that target WASI interfaces.
However, {admc-short-name} policies use the https://github.com/wapc[waPC] project to implement bi-directional communication between the policy and the policy runtime (`kwctl` or `policy-server`).
{admc-short-name} use of the communication protocol is described xref:reference/spec/01-intro-spec.adoc[here].
...
{admc-short-name} supports WASI policies from the {admc-short-name} 1.7.0 release forward.
====
...
Currently, only the {admc-short-name} Go and JavaScript/TypeScript SDKs expose them to
WASI policies.
...
The {admc-short-name} project team advise that you write {admc-short-name} Go policies using the TinyGo compiler, as described xref:tutorials/writing-policies/go/01-intro-go.adoc[here].
...
Each {admc-short-name} policy must be annotated via the `kwctl annotate` command.
