...
A {admc-short-name} policy exposes two different functions that receive the policy settings as input:
...
object.
* `validate_settings`: Call this function when the policy is first loaded by {admc-short-name}.
...
All {admc-short-name} policies have to implement settings validation.
...
https://github.com/kubewarden/policy-sdk-go[{admc-short-name}'s SDK].
