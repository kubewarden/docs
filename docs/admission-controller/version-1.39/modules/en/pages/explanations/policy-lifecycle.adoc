...
The controller keeps the policy object in the cluster, but does not
deploy the policy:

* No webhook configuration is registered
* No admission requests are forwarded to the policy
* The policy is not audited by the audit scanner
* The controller removes the policy from the `+PolicyServer+`
configuration
...
policy allowed resources] for the allow list and its default value.
...
remove a resource from the allow list, a policy with the `+active+`
...
stateDiagram-v2
    ...
    unscheduled --> rejected : resource outside allow list
    scheduled --> rejected : resource outside allow list
    pending --> rejected : resource outside allow list
    active --> rejected : resource leaves allow list
    rejected --> pending : resource added to allow list
....
