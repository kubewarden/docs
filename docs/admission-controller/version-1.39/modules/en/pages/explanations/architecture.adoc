...
{admc-name} is a Kubernetes policy engine.
It uses policies written in a programming language of your choosing.
This language must generate a WebAssembly binary for {admc-name} to use.
...
{admc-name} uses the terms 'PolicyServer' when discussing the {admc-name}
policy server and `policy-server` when discussing Pod or Deployment of a
{admc-name} PolicyServer.
...
The team designed {admc-name} to use core features of Kubernetes,
...
{admc-name} operates seamlessly within the Kubernetes ecosystem. At its
core, the {admc-name} controller is a Kubernetes controller, monitoring
{admc-name} Custom Resource Definitions (CRDs) and configuring Kubernetes
resources to execute them. This integration ensures that {admc-name} uses
...
{admc-name} employs CRDs to define and manage {admc-name} resources,
...
When setup by the {admc-name} controller, the policy-server Service receives
...
ensures that {admc-name} policies are versatile, so you can distribute and
run them on diverse Kubernetes clusters

=== OCI based policy artifacts

Policies in {admc-name} are OCI (Open Container Initiative) artifacts. This
...
{admc-name} associates policies with their own 'validation' or 'mutating'
...
{admc-name} consists of these components:

* {admc-name} Custom Resources are
...
+
{admc-name} integrates with Kubernetes using
https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/[Dynamic
Admission Control]. In particular, {admc-name} operates as a Kubernetes
Admission Webhook. The `policy-server` is the Webhook endpoint called by the
Kubernetes API server to validate requests.

* The https://github.com/kubewarden/adm-controller[{admc-name}
controller] is a Kubernetes controller that reconciles {admc-name}'s
Custom Resources. This controller creates parts of the {admc-name} stack.
It also translates {admc-name} configuration into Kubernetes directives.
+
The admission controller registers the needed
`MutatingWebhookConfiguration` or `ValidatingWebhookConfiguration` objects
with the Kubernetes API server.

* xref:/tutorials/writing-policies/index.adoc[{admc-name} policies] are
...
* The xref:/reference/policy-server-cli.adoc[PolicyServer] receives
requests for validation. It validates the requests by executing
{admc-name} policies.

* The xref:/explanations/audit-scanner/audit-scanner.adoc[audit scanner] inspects the
...
longer adhere to the deployed {admc-name} policies.
+
.Architecture
include::latest@shared:en:partial$architecture.adoc[]

== The journey of a {admc-name} policy

=== Default PolicyServer

On a new cluster, the {admc-name} components defined are:
...
{admc-name} works as a Kubernetes Admission Webhook. Kubernetes specifies
...
and downloads all the {admc-name} policies specified. You can download
{admc-name} policies from remote HTTP servers and container registries.
...
When {admc-name} has configured all policies, the `policy-server` spawns a
pool of worker threads to evaluate incoming requests using the {admc-name}
policies specified by the user.

Finally, the `policy-server` starts a HTTPS server, listening to incoming
validation requests. {admc-name} uses the TLS key and certificate created by
the {admc-name} controller to secure the web server.
...
Once {admc-name} marks the `policy-server` deployment as 'uniquely
reachable' or `Ready`, the `admission-controller` makes the Kubernetes API
...
{admc-name} evaluates each policy inside its own dedicated WebAssembly
...
== How {admc-name} handles many PolicyServer and policies

A cluster can have many PolicyServers and {admc-name} policies defined.
