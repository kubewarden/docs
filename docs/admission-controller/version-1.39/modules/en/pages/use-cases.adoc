...
:sidebar_position: 74
:description: Learn about various use cases for {admc-name} in different Kubernetes environments and personas, including multiple scenarios.
:keywords: {admc-name}, documentation, use cases, case studies
:doc-persona: kubewarden-all
:doc-type: explanation
:doc-topic: explanation

These are a few example use cases for {admc-name}'s xref:/personas.adoc[personas].

== Case A: As a Kubernetes operator, I want to ensure my cluster is safe and compliant.

I deploy {admc-name} Helm chart, in the `kubewarden` Namespace. This deploys a
default PolicyServer. I enable the recommended ClusterAdmissionPolicies in the
...
cluster may no longer be so. Yet {admc-short-name} has already accepted those
...
Operators can receive information from policies and the {admc-short-name} stack by
...
As an operator, I deploy {admc-short-name} as in Case A for a set of policies of my
...
{admc-short-name} achieves this by supporting any language that compiles to
...
that compiles to Wasm). {admc-short-name} provides
xref:/tutorials/writing-policies/index.adoc[SDKs] for a few languages as
first-class support.

{admc-short-name} policies can be simple or complex
...
== Case D: As a system integrator, I want to re-use {admc-short-name} as part of my security and compliance solution.

As a system integrator, I want to re-use {admc-short-name}, and possibly re-use other
solutions by including them in {admc-short-name}.

As a system integrator, I can reuse parts of {admc-short-name}. For example, the
...
System integrators can choose to deploy the `admission-controller` or manage
...
{admc-short-name} doesn't intend to:

- Replace Kubernetes built-in security features, but complement them:
  - {admc-short-name} provides migration from PSPs.
  - You can re-use ValidatingAdmissionPolicies and CEL policies with {admc-short-name}'s
    `cel-policy`.
  - {admc-short-name} policies can be mutating, while Pod Security Admission cannot.
  - {admc-short-name} policies benefit from the {admc-short-name} stack features (audit
