...
* https://jfrog.com/artifactory/[JFrog Artifactory]. See https://docs.jfrog.com/artifactory/docs/oci-repositories[here].

== Tools that work with OCI registries

The {admc-short-name} project recommends:

* xref:/reference/kwctl-cli.adoc[`kwctl`] (our CLI tool).
...
Currently, Docker Hub doesn't support OCI artifacts so can't be used to store {admc-short-name} policies.
...
https://github.com/kubewarden/adm-controller/issues/59[Read more here].
