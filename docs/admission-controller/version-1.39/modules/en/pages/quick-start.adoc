...
:description: Understand the {admc-name} stack by learning how to deploy and manage its main components, including PolicyServers and ClusterAdmissionPolicy resources.
:doc-persona: ["kubewarden-all"]
:doc-topic: ["quick-start"]
:doc-type: ["tutorial"]
:keywords: ["{admc-name}", "installation", "quick start", "policyserver", "clusteradmissionpolicy", "admissionpolicy"]
...
{admc-short-name} describes its Kubernetes Custom Resource Definitions (CRDs)
xref:reference/CRDs.adoc[here].

{admc-short-name} CRDs mentioned in this tutorial and in the rest of
...
The {admc-short-name} has three main components which you interact with:
...
The {cluster-admission-policy} resource is the core of the {admc-short-name}
...
| Y
| `module`
| The location of the {admc-short-name} policy. The following schemes are allowed:
...
An {admission-policy} can target only the resources that the resource
allow list names. Cluster administrators control this list.
Cluster wide policies do not have this limit. See
xref:/howtos/security-hardening/security-hardening.adoc#_namespaced_policy_allowed_resources[Namespaced
policy allowed resources] for more information.

[IMPORTANT]
====

{admission-policy} requires Kubernetes 1.21.0 or greater. This is because
{admc-short-name} uses the `kubernetes.io/metadata.name` label, introduced in
Kubernetes 1.21.0

====

The complete documentation of these Custom Resources is xref:/reference/CRDs.adoc[here].
...
* Managed defaults such as the PolicyServer `default` and recommended policies
...
Now, you are ready to learn more about the {admc-short-name}! Have a look at the policies on
...
.Small selection of available policies on ArtifactHub
