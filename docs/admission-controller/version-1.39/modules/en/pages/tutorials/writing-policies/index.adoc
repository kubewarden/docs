...
This section introduces the {admc-name} policies using traditional computing analogies.

An {admc-short-name} policy is like a program that does one job.
...
The xref:reference/policy-server-cli.adoc[policy-server]
component of {admc-short-name} performs these operations.

The policy server doesn't include any data processing capability.
You add processing capabilities at runtime with add-ons.
These add-ons are the {admc-short-name} policies.

So, a {admc-short-name} policy is like a
https://en.wikipedia.org/wiki/Plug-in_%28computing%29[traditional plug-in]
of the "policy server" program.

To recap:

* {admc-short-name} policies are plug-ins that expose a set of well-defined
...
Writing {admc-short-name} policies consists of writing the validation business logic and then exposing it through a well-defined API.

== Programming language requirements

You supply {admc-short-name} policies as
...
To overcome this limitation {admc-short-name} policies use the https://github.com/wapc[waPC] project, which provides a bi-directional communication channel.
