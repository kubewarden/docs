...
This guide will show you how to enable mutual TLS (mTLS) for the all the webhooks used by the Kubewarden
stack when using [k3s](https://k3s.io/) as your Kubernetes distribution.

## Prerequisites

Before installing k3s, you need to create a certificate authority (CA) and a client certificate that
will be used to secure the communication between the Kubewarden webhooks and the Kubernetes API server.
...
As `root` user, move into the `/etc/rancher/k3s/admission/certs` directory and
...
have to be put into a `ConfigMap` under the `kubewarden` namespace. The contents of the `rootCA.crt` file
...
Then create the `ConfigMap` inside of it. The following command, run on the Kubernetes node,
...
The resulting `ConfigMap` will be named `api-server-mtls`.

### Install the Kubewarden stack

Install the Kubewarden stack as reported in the [quickstart guide](../../quick-start.md).
...
Once this command completes, the Kubewarden stack will be installed and all its webhooks will be secured with mTLS.
