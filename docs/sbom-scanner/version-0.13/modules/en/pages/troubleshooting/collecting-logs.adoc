...
The `DaemonSets` section is skipped when node scanning is disabled in the chart (`controller.nodeScan.enabled=false`).
...
* Logs from the pods of all {sboms-name} components (controller, workers, node workers, storage, NATS), including init containers and previous-container logs for crash-looped pods.
