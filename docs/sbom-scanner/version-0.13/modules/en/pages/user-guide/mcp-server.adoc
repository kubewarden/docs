...
[NOTE]
====
This feature is experimental. Future releases can change or remove it.
====

The MCP server lets AI assistants interact with {sboms-name}. They can query
vulnerability data and manage scanning resources. Any client that supports the
MCP protocol can connect.
...
The commands show the MCP pod and a Service on port 8222.
...
To reach the MCP server from a local machine, forward the in-cluster Service:
...
[NOTE]
====
For production use, expose the MCP server through an Ingress or LoadBalancer.
====
...
[NOTE]
====
Use the `_summary` tools instead of the full `_vulnerabilities` tools. Summary
tools return severity counts and the 10 most severe CVEs. Full tools can return
responses that exceed client context limits.
====
...
After the connection succeeds, use natural language to interact with
{sboms-name}. These prompts are organized by workflow.
...
Use read-only mode to let AI assistants query vulnerability data without
modifying {sboms-name} resources.

[#_rate-limiting]
== 8. Rate Limiting

The MCP server enforces a limit of 10 requests per second. It permits bursts
of 50 requests. Normal conversational use does not reach this limit.
...
To disable TLS for development or external TLS termination, use this command:
...
[WARNING]
====
Do not disable TLS in untrusted network environments. Disabling TLS sends
Basic Auth credentials as plaintext.
====
