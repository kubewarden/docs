...
. On the nodes matching the node selector, the DaemonSet pod scans the
  host filesystem.
. The scan produces `NodeSBOM` (in SPDX format) and `NodeVulnerabilityReport`
  resources.

=== Default behavior

The Node Scan reconcilers are enabled by default when installing
{sboms-name} via the Helm chart. Node scanning becomes active once you
...
If unset, container-runtime directories are skipped by https://docs.kubewarden.io/sbom-scanner/0.12.0/en/reference/crd-reference.html#k8s-api-github-com-kubewarden-sbomscanner-api-v1alpha1-nodescanconfiguration[default].
Set to an empty list to scan everything, including the paths above.of skip patterns is provided by default to avoid scanning container runtime directories.
