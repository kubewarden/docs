...
The Kubewarden stack uses different webhooks to enforce policies on the Kubernetes cluster. Each `PolicyServer` instance exposes a webhook
...
To decrease their attack surface, access to these webhooks should be limited to the only valid callers they have: the Kubernetes API server
and the [audit scanner](../../explanations/audit-scanner/) component.
This can be done by using network policies and authentication independently or in conjunction with each other to harden the webhooks against attacks.

## Block External Traffic Using Network Policies

The webhooks are only expected to accept requests from the Kubernetes API server and from the audit scanner component. By default, however, the webhooks
can accept traffic from any source.
...
spec:
  ...
  ingress:
    ...
    - action: Allow
      ...
      destination:
        selector:
          'app.kubernetes.io/name == "kubewarden-controller" || has(kubewarden/policy-server)'
...
Use the CiliumNetworkPolicy resource in the `cilium.io/v2` API group, define a network policy like the following one:
...
can access them.
More information can be found in the [Kubernetes documentation](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/#authenticate-apiservers).
...
1. Configure the API server to present a client certificate to the webhook, pointing to an `AdmissionConfiguration`
    ...
    This is also the same config file where other admission plugins are configured, such as `PodSecurity`.
    If your distro or your setup uses additional admission plugins, configure those as well.

2. Create the `kubeconfig` file that the admission plugins refer to. Kubewarden only supports client certificate authentication, so generate a TLS key pair,
...
3. Start the `kube-apiserver` binary with the flag `--admission-control-config-file` pointing to your `AdmissionConfiguration` file.
   The way to do this varies by distro, and it isn't supported universally, such as in hosted Kubernetes providers.
...
4. The certificate of the root CA that issued the API server client certificate must be made available to
   ...
   Its content have to be put into a `ConfigMap` under the `kubewarden` namespace using a key named `client-ca.crt`.
...
5. Finally, when installing the `kubewarden-controller` Helm chart,
   make sure to enable the following values:
    ...
    The Kubewarden controller will create a client certificate to be used by the audit scanner component.
    The certificate will be automatically rotated by the controller when needed.
