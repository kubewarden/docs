...
:description: Description of some common tasks undertaken after installing {project-name}. Provides examples of these tasks using kwctl
...
This describes tasks that can be performed after you xref:quick-start.adoc#installation[install {project-name}] in your Kubernetes cluster.

Each task can be done separately; they are shown here in a logical ordering.

== Test Policies

{short-project-name} has two tools to help you find policies and test them locally:

* https://artifacthub.io/packages/search?kind=13&sort=relevance&page=1[Artifact Hub] using their package filter for {short-project-name} policies.
* https://github.com/kubewarden/kwctl[`kwctl`] CLI tool

=== Artifact hub

Artifact hub hosts policies contributed by the community. For example, you can find substitutes to the https://kubernetes.io/blog/2021/04/06/podsecuritypolicy-deprecation-past-present-and-future/[deprecated Kubernetes Pod Security Policies], created by the {short-project-name} developers.
...
* xref:tutorials/writing-policies/index.adoc[Writing Policies] explains how to write policies in different languages and generate WebAssembly binaries, so they can be used by {short-project-name}.
