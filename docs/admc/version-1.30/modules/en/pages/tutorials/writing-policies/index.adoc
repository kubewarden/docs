...
= Writing {project-name} policies
:revdate: 2025-07-09
:page-revdate: {revdate}
:description: An introduction to writing {project-name} policies.
...
This section introduces {project-name} policies using traditional computing analogies.

A {short-project-name} policy is like a program that does one job.
...
component of {short-project-name} performs these operations.

The policy server doesn't include any data processing capability.
You add processing capabilities at runtime with add-ons.
These add-ons are the {short-project-name} policies.

So, a {short-project-name} policy is like a
https://en.wikipedia.org/wiki/Plug-in_%28computing%29[traditional plug-in]
of the "policy server" program.

To recap:

* {short-project-name} policies are plug-ins that expose a set of well-defined
...
Writing {short-project-name} policies consists of writing the validation business logic and then exposing it through a well-defined API.

== Programming language requirements

You supply {short-project-name} policies as
...
To overcome this limitation {short-project-name} policies use the https://github.com/wapc[waPC] project, which provides a bi-directional communication channel.
