...
:description: Configuring PolicyServers to use a private or self-hosted Sigstore instance for policy signature verification in {admc-name}.
...
Sigstore instance. Otherwise, you may end up configuring {admc-name}
...
{admc-short-name} looks for this exact key.
====
