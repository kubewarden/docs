...
{admc-name} provides a CEL policy that is capable of running Kubernetes VAP policies without any modifications.
You can read more about the CEL policy in xref:../tutorials/writing-policies/CEL/01-intro-cel.adoc[this section] of {admc-short-name}'s documentation.
...
explains the benefits of running VAP policies using {admc-short-name}.

This guide explains how to use the `kwctl` tool to migrate a VAP policy to {admc-short-name}.
...
You can migrate the policy to {admc-short-name} by following these steps:
