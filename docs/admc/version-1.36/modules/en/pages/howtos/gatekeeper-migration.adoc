...
{admc-name} policy. This process involves two main steps:
. Compile the Rego program into a WebAssembly (Wasm) module.
. Distribute the WebAssembly module as a {admc-short-name} policy.
...
into a functional {admc-short-name} policy. It uses a basic Gatekeeper demo policy.
...
* xref:/howtos/install-kwctl.adoc[`kwctl`]: tool you use to
  prepare and run {admc-short-name} web assembly module
...
already available policies in the {admc-short-name}
...
available OPA and Gatekeeper policies migrated to {admc-short-name}.

Also, take a look at our
xref:/explanations/comparisons/opa-comparison.adoc[comparison]
documentation between {admc-short-name} and Gatekeeper.

== Step 1: Initialize Your {admc-short-name} policy project

First, use the {admc-short-name} Gatekeeper
...
`ConstraintTemplate` and its associated `Constraint` resources into a {admc-short-name}
policy. In a {admc-short-name} context, consider the `ConstraintTemplate` as the core
policy code, while the `Constraint` instances translate into policy instances
running within {admc-short-name}.

First, copy the Rego code from your `ConstraintTemplate` into the `policy.rego`
file the {admc-short-name} template generated. For this example, you should use the
following basic
...
=== Adapt Rego code for {admc-short-name}

You need to make sure the `package` name used inside of the Rego code is `policy`.
This is the value expected in many places by the {admc-short-name} Gatekeeper template.
...
The {admc-short-name} Gatekeeper template includes both Rego unit tests and end-to-end
...
`Constraint`. You need to update the `settings` section in the generated {admc-short-name}
