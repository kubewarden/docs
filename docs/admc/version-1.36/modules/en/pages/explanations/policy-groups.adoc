...
:doctype: book
:keywords: ["{admc-name}", "kubewarden", "policy groups", "clusteradmissionpolicygroup", "admissionpolicygroup"]
...
The policies field is a map of ordinary policies. {admc-name} calls policies by
...
ordinary {admc-short-name} policies, containing only the `module`, `settings` and
