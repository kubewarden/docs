...
Rego policies support context aware data from the {admc-name} 1.9 release.
====
...
If you're more interested in the {admc-short-name} implementation,
...
starting from the {admc-short-name} 1.9 release.
...
but {admc-short-name} hasn't yet implemented mutating policies with Gatekeeper compatibility.
You can use policies that use the {admc-short-name} SDK to write mutating policies,
but currently, you can't run Gatekeeper mutating policies in {admc-short-name}.
