...
{admc-name} uses https://github.com/bytecodealliance/javy[Javy] (a Bytecode Alliance project) to build WebAssembly binaries from JavaScript and TypeScript.
...
The {admc-short-name} project currently uses Javy for these reasons:
...
Despite these limitations, Javy provides sufficient capabilities for writing effective {admc-short-name} validation policies through the hosts capabilities system.
...
Writing {admc-short-name} policies requires:
...
These TypeScript/JavaScript libraries are useful when writing a {admc-short-name} policy:

* https://github.com/kubewarden/policy-sdk-js[{admc-short-name} JavaScript SDK]: Provides structures and functions reducing the amount of code necessary. It also provides test helpers and access to all host capabilities.
...
The {admc-short-name} project provides a https://github.com/kubewarden/js-policy-template[template JavaScript/TypeScript policy project] you can use to create {admc-short-name} policies.

== Getting the toolchain

The easiest way to get the toolchain is by using the {admc-short-name} JavaScript SDK, which includes the Javy compilation plug-in:
...
* https://github.com/kubewarden/kwctl/releases[`kwctl` ≥ v1.30]: CLI tool provided by {admc-short-name} to run its policies outside of Kubernetes, among other actions. It's covered in xref:/tutorials/testing-policies/index.adoc[the testing policies section] of the documentation.
