include::partial$variables.adoc[]
= Other languages
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-05-04
:page-revdate: {revdate}
:description: Learn about writing {admc-name} policies using various programming languages that target WebAssembly modules and the WASI interface.
:doc-persona: ["kubewarden-policy-developer"]
:doc-topic: ["kubewarden", "writing-policies", "other-languages"]
:doc-type: ["tutorial"]
:keywords: ["kubewarden", "kubernetes", "writing policies", "other languages"]
:sidebar_label: Other languages
:sidebar_position: 70
:current-version: {page-origin-branch}

{admc-name} Policies can be written with any programming language that can produce
WebAssembly modules targeting the WASI interface.

Please open a Pull Request or a GitHub Issue https://github.com/kubewarden/docs[here]
if you know of a programming language that can be used to write {admc-short-name} Policies.
