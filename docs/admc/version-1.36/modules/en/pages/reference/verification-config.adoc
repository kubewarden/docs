...
* `githubAction`: For signatures performed with Sigstore's keyless workflow
  inside GitHub Actions. {admc-short-name} checks this information against the x509
...
With {admc-short-name}, you can verify policy signature are from trusted users *and*
...
This last example tests if a given policy came from the {admc-short-name}
organization:
